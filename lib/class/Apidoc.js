...
function _route (properties, onCreate) {
  ...
  if (properties.paramExamples) {
    properties.paramExamples.forEach(paramExample => {
      const example = _customExample(paramExample.data)
      apidoc += `* @apiParamExample {json} ${paramExample.title} \n${example}`
    })
  }
  ...
  if (properties.successExamples) {
    properties.successExamples.forEach(successExample => {
      const example = _customExample(successExample.data)
      apidoc += `* @apiSuccessExample {json} ${successExample.title} \n${example}`
    })
  }
  apidoc += `*/\n`
...
function _apidocType (field, onlyType) {
  ...
  if (field.type.key === 'ENUM') {
    if (onlyType === true) { return 'String' }
    return `String=${field.type.values.toString()}`
  }
